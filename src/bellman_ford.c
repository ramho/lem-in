...
void try_reduce(char *pre, char *sec, int w, t_lemin *lemin)
{
	...
	if (lemin->node_tab[pre_index]->infinity == 0 && lemin->node_tab[sec_index]->infinity == 1) // if not yet passed to the second node
	{
		lemin->node_tab[sec_index]->reach_cost = lemin->node_tab[pre_index]->reach_cost + w;
		lemin->node_tab[sec_index]->infinity = 0;
		lemin->node_tab[sec_index]->predecessor = ft_strdup(pre);
	}
	if (lemin->node_tab[pre_index]->infinity  == 0 && lemin->node_tab[sec_index]->infinity == 0) // if both nodes have already been connected to a previous one
	{
		if (lemin->node_tab[sec_index]->reach_cost > (lemin->node_tab[pre_index]->reach_cost + w))
		{
			lemin->node_tab[sec_index]->reach_cost = lemin->node_tab[pre_index]->reach_cost + w;
